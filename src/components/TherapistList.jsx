...
export default function TherapistList({ authUserId }) {
  ...
  const [isLocked, setIsLocked] = useState(false) // unlock for all users

  useEffect(() => {
    const init = async () => {
      // 1. Load real DB therapists — mark all as available
      const { data: tData } = await getTherapists()
      if (tData && tData.length > 0) {
        const normalized = tData.map(t => ({
          ...t,
          is_available: true,   // real accounts are always available
          spec: t.specialization,
          rating: 4.8,
          reviews: Math.floor(Math.random() * 300) + 100,
          avatar: t.name?.includes('Dr. S') || t.name?.includes('Dr. P') || t.name?.includes('Dr. E') || t.name?.includes('Dr. O') ? '👩‍⚕️' : '👨‍⚕️',
          tags: typeof t.tags === 'string' ? JSON.parse(t.tags) : (t.tags || [t.specialization]),
          exp: '8+ years',
          price: 'Free'
        }))
      ...
      // 2. Check if user already has an assigned therapist
      if (authUserId) {
        const { data: assignment } = await getAssignedTherapist(authUserId)
        if (assignment?.therapists) setAssignedTherapist(assignment.therapists)
      }
  ...
  const handleAssign = async () => {
    if (!authUserId || !selected) return
    setAssigning(true)
    const { error } = await assignTherapist(authUserId, selected.id)
    if (!error) {
      setAssignedTherapist(selected)
      setSelected(null)
    }
    setAssigning(false)
  }
  ...
  if (activeChatTherapist) {
    return <UserTherapistChat userId={authUserId} therapist={activeChatTherapist} onClose={() => setActiveChatTherapist(null)} />
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        {assignedTherapist && (
          <div className="mt-3 inline-flex items-center gap-2 text-xs font-medium bg-emerald-500/10 text-emerald-400 px-3 py-1.5 rounded-lg border border-emerald-500/20">
            <CheckCircle size={14} /> Connected with {assignedTherapist.name}
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((t, i) => {
          const isAssigned = assignedTherapist?.id === t.id
          return (
            <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className={`glass rounded-2xl p-5 card-hover border ${isAssigned ? 'border-emerald-500/30' : 'border-white/5'}`}>
              ...
              <div className="flex gap-2">
                {isAssigned ? (
                  ...
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                    ...
                    className="flex-1 py-2 rounded-xl text-sm font-medium bg-gradient-to-r from-blue-600 to-purple-600 text-white hover:shadow-lg hover:shadow-blue-500/20 flex items-center justify-center gap-1"
                  >
                    <UserCheck size={13} /> Connect
                  </motion.button>
                )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-white">Connect with Therapist</h3>
              ...
              <p className="text-sm text-slate-400 mb-4">This therapist will be assigned to guide your wellness journey. You can chat with them directly after connecting.</p>
              <button
                onClick={handleAssign}
                disabled={assigning}
                className="w-full py-3 rounded-xl bg-gradient-to-r from-teal-600 to-blue-600 text-white font-semibold text-sm disabled:opacity-50"
              >
                {assigning ? 'Connecting...' : `✅ Connect with ${selected.name}`}
              </button>
            </motion.div>
          </motion.div>
        )}
